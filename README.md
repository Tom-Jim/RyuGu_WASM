...
The page starts the Bevy frontend WASM (`pkg/ryugu_wasm_bg.wasm`) first, then spawns the dedicated numerical Worker (`src/html/backend-worker.js`), which instantiates the page's only Zig-built C++ module (`pkg/ryugu_backend.wasm`) and independent Rust backend WASM (`pkg/backend/ryugu_backend_bg.wasm`). No numerical module is instantiated on the main thread and no synchronous numeric WASM call remains there. The JavaScript host bridges the independent linear memories with typed arrays; no native numerical service, WebSocket backend, or shared-memory support is required.

Every numeric call is posted to the Worker as an immutable payload: geometry configuration, live probe advancement, pointwise Jacobi field evaluation, C++ FMM/FFT planning slices, candidate trajectory propagation, density solves, the surface-field overlay stencils, the Basilisk comparison oracle, the f64 planning reference, and the inversion reference/sensitivity matrices. Each call site has its own frontend channel (`cpp_backend::Backend*Channel`): a request sets `in_flight`, the Worker's `postMessage` reply is pushed into the channel by an exported `deliver_backend_*_result` function, and Bevy polls the channel like a GPU readback. Replies carrying a stale `request_id`/`epoch` are dropped; a method switch, probe edit, or crash reset clears every channel so no request can stay in flight forever. Consumers keep their last completed result while a request is outstanding and skip the frame rather than block. The advance path is paced only by the outstanding request plus a 16 ms floor, FMM/FFT planning slices are additionally rate-limited because each rebuilds its tree/grid, and the comparison oracle is paced by its outstanding request alone (one reference per new physics sample). `node src/tools/check_wasm_build.mjs` verifies statically that every request kind the frontend posts is executed by the Worker and routed back by the page.
...
First/Stress preparation batches candidate targets through C++ ExaFMM at each integration time. The dedicated browser Worker owns the Rust and C++ WASM instances, so a long FMM slice cannot block Bevy rendering or the control panel. Within a time slice, the endpoint acceleration is reused for the next initial kick at the identical position and time; no linearized field replaces FMM. A slice of k integration steps requires k+1 FMM evaluations instead of 2k (for k >= 1). The canonical source geometry is uploaded once per planning run (`prepare_candidate_sources`, acknowledged on the candidates channel before the first slice is sent) and released when the run ends, is cancelled, or is replaced (`clear_candidate_sources`); slices carry only candidate states, and the Worker's serialized queue guarantees a slice never runs against sources uploaded for another run. Target, output, and result buffers are reused, and scale-matched FMM translation matrices are cached. The browser OpenMP shim is single-threaded, so target batching does not enable native CPU threads or GPU execution.
...
FLUPS's negative potential is converted to the positive potential convention above without reversing acceleration. The current live grid is 32 cubed with a 4096 metre half-width; out-of-domain targets fail explicitly. Scale-matched ExaFMM translation matrices persist across planning slices and the source nodes stay resident in the Worker's C++ module for the whole planning run, while target-dependent tree topology is still rebuilt for each slice. Basilisk polyhedral preprocessing is cached per configured mesh. These costs and grid errors still require runtime evaluation; compilation is not evidence of performance parity.
...
The local server runs at `http://localhost:3000`. The primary command is `bun start` (Bun's shortcut for `bun run start`). `bun .` and `bun run .` run the same `package.json` `module` entry: incremental WASM, then the server. Bun 1.3 has no bunfig default-script key; a bare `bun run` only lists scripts. `bun run preview` is an alias of `start`. `bun run dev` uses a development frontend WASM package. Keep the supplied model and operator assets with the project. Set `PORT=3001` to use another local port. The server serves `.mjs` as JavaScript and `.wasm` as WebAssembly.

Missing C++ checkouts are fetched automatically from revisions pinned in `C++/sources.lock.json`; existing checkouts are not overwritten. `bun run fetch:cpp` explicitly verifies pins and applies versioned browser patches. The official FFTW development repository is retained in `C++/fftw3`; compilation uses the pinned release-source mirror in `C++/fftw3-release` for generated codelets. CMake writes FFTW and FLUPS objects under `target/cpp-wasm` rather than into the vendor trees. First builds require network access. `bun run build` incrementally compiles all three modules without launching a browser, server, or numerical tests.
