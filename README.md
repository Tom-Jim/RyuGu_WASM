...
The planning contract is separate from the 56-variable Clarabel density inversion above. It resamples the current immutable trajectory capture with the same quintic-Hermite curve used by inversion, freezes that actual short arc, and generates exactly `2048` complete candidate trajectories in its Frenet-like transverse tube. Every point carries body-fixed position and velocity, sample time, body rotation, candidate/sample identity, and measured transverse distance; generation fails if any point exceeds the shared `15 m` trust and transverse limit. Eq.106 uses fourth-order spectral elements no longer than `300 s` and a relative Taylor remainder target of `1e-3`. The selectable workloads are:

| Profile | Candidate trajectories | Density models | Samples per candidate | Evaluations |
|---|---:|---:|---:|---:|
| First | 2,048 | 16 | 241 | 7,897,088 |
| Stress | 2,048 | 32 | 512 | 33,554,432 |

The upper-right comparison panel keeps `Density fit` and `Inversion time` and adds gravity error, gradient error, propagated pericenter error, minimum altitude, density-model separation, the verified planning objective, Eq.106 segment count, speedup versus GPU FMM, and cold-start amortization. The `K x 56` density rows contain genuinely different center, shell, lobe, quadrupole, and rubble patterns and are normalized to the same total mass. Planning results are accepted only when Eq.106, MMFFT, and FMM carry the same nonzero capture, source geometry, voxel basis, candidate-state, density-model, and sample-array hashes; use the selected `B x K x H` workload; and identify their actual GPU backends. Missing, mismatched, or failed GPU results remain `N/A`, and speedup/winner fields stay locked until all three verified rows exist.
...
Selecting `First` or `Stress` starts a new planning run identity, and `Invert trajectory` queues that workload against the current frozen capture. Candidate states, all structured density rows, voxel geometry, and the common 1024-source basis are uploaded once as immutable shared GPU buffers. Eq.106 runs its real line-sample, spectrum, analytic-coefficient, and field-evaluation passes; MMFFT samples its real two-level zero-padded convolution hierarchy; FMM evaluates its real order-two octree and exact leaf interactions. All three produce the same field/Jacobian contract in 16-candidate tiles. A common GPU reduction pass then emits one compact metric row per candidate, while only the deterministic direct-verification field rows are copied to mapped staging; no complete `B x K x H` result tensor is retained or read back.

Common trajectory/density preparation is reported separately and excluded from every method total. Method preprocessing, command encoding and GPU completion, reduction, compact mapped readback, dispatch count, and actual kernel-evaluation count are reported separately. Independent direct verification is also timed separately and excluded from the winner timing. A final hot-payload tile supplies warm timing; source/tree/grid/operator buffers are reused rather than re-uploaded for every tile. Eq.106 caches the identical spectral-element partition for every density row, while MMFFT reuses its RustFFT plans, Newton-kernel spectra, and workspaces across density rows within one frozen batch. Accuracy is checked on a deterministic candidate/model/time subset with an independent direct 1024-source sum, including an analytic spatial gradient; readback failures return an explicit failed result instead of leaving the UI pending forever.
