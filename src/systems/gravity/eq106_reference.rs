...
/// Builds the complete one-parameter Taylor jet of the density-summed
/// acceleration along `observer(t) = center + t * displacement`.
///
/// Each source contribution is expanded analytically as
/// `(r0 - t*d) * |r0 - t*d|^-3`. This produces every directional derivative
/// required by Eq. (118), rather than silently applying only its first term.
...
/// Constructs a diagonal [m/m] Pade continuation from the actual Eq. (118)
/// directional Taylor jet. Acceptance requires all three component
/// denominators to be pole-free on the traversed interval, agreement with the
/// adjacent [m-1/m-1] approximation, and an independent field residual at the
/// requested endpoint.
...
pub fn certify_eq106_line(
    ...
    // The line certificate is a runtime gate, not the high-precision unit
    // quadrature. Use a bounded numerical tolerance for the 257 shared
    // frequencies; the independent direct-field residual and the separate
    // boundary-identity probes remain the acceptance checks.
    ...
    for &h in h_values {
        ...
        // The source mass can be tiny in unit tests; use a physical absolute
        // floor so an endpoint quadrature residual is not amplified into a
        // false rejection by dividing by a sub-normal potential difference.
