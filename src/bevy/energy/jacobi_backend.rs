...
pub fn record_probe_jacobi_system(
    ...
    // The four pointwise methods are now evaluated by the C++ WASM backend.
    ...
    if *active_method != ActiveGravityMethod::FrequencyDomain {
        if !cpp_backend.ready || !live_pacer.allow() {
            return;
        }
        ...
        let Ok((_, positive_potential)) = crate::cpp_backend::evaluate(*active_method, body_position)
        else {
            return;
        };
        ...
        history.last_request_id = Some(clock.request_id);
