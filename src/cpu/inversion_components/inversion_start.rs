pub fn start_density_inversion_system(
    ...
    if method == ActiveGravityMethod::FrequencyDomain {
        let Some((training, training_basis, holdout, holdout_basis)) =
            ...
            inversion.error = Some("The frequency-domain observation operator could not be assembled.".into());
            return;
        };
...
fn prepare_frequency_domain_cache(
    ...
    if !cache_hit {
        ...
        sensitivity.source_hash = source_hash;
        sensitivity.basis_hash = basis_hash;
        sensitivity.configuration_hash = crate::gpu::frequency_domain::frequency_domain_sensitivity_configuration_hash();
        sensitivity.voxel_count = voxel_count;
