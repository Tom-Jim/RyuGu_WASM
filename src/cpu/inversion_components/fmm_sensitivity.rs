...
//     }
    pub(crate) fn sources(&self) -> &[(DVec3, f64)] { &self.0.0 }
...
//
//
// Source storage only; field evaluation is delegated to the C++ module.
struct FmmNode(Vec<(DVec3, f64)>);

fn build_fmm_node(points: Vec<(DVec3, f64)>, _depth: u8, _maximum_depth: u8) -> FmmNode {
    FmmNode(points)
}
